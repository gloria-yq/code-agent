...
- 本地 Web 应用使用 `web` 模式，先拒绝已占用端口，启动后等待指定地址返回有效 HTTP 响应，再打开默认浏览器；
- 原生桌面程序使用 `desktop` 模式，启动其 GUI 进程。

Web 预览只接受 `localhost`、`127.0.0.1` 或 `::1` 地址，不会代替用户打开任意外部网址。端口冲突会作为工具错误返回模型，以便改用其他端口，而不会误开已有服务。启动命令使用所选工作区作为目录，子进程不会继承模型 API key。TUI 中可输入 `/processes` 查看运行状态、最近日志、重新打开 Web 页面或停止程序。切换工作区、断开模型连接、退出 TUI 或解释器经正常错误路径退出时，会停止仍由当前运行时管理的进程树。
