...
`ProcessManager` 管理。它为交互式 CLI 打开系统终端；本地 Web 服务会先检查端口未被占用，
再要求新进程返回有效 HTTP 响应后打开浏览器；桌面程序则启动 GUI 进程。管理器记录稳定的
进程 ID、工作目录、状态、URL 和受限日志，并向模型暴露启动、列举、重开预览和停止四个窄接口。

进程管理器属于工作区运行时：同一工作区重建模型连接时复用，切换工作区时不复用。TUI 在
切换、断开或退出前处理仍运行的进程，并注册解释器退出兜底清理，避免正常错误路径遗留
进程。所有子进程都使用剥离模型凭据后的环境；Web 预览只允许回环地址。这一层负责生命周期
和凭据边界，不构成操作系统沙箱。
